...
- Selection-based translation with floating icon (or hotkey-only mode). The card is always the same size, wherever on the page you select: the original and the translation each scroll in their own half of it, so nothing moves while an answer streams in. That size is yours to set in General → Appearance. A card still waiting for its first words shows only that, and grows to the frame when they arrive. It names the languages it is working between, shows the source above the translation, and carries its own controls: switch provider or target language for that card alone, copy the result, or ask again. Those choices last as long as the card and are never written back to your settings. It can be pinned so it keeps its place while you scroll and clicks elsewhere don't dismiss it, and drags by its grip when it lands on top of what you were reading.
...
- Providers are a searchable panel in Settings → Translation: switch one on, open it to fill in endpoint, key and model, and it reports its own latency from that row. Vendors with more than one region or plan offer those as a choice of endpoint — opencode's Zen and Go, and Qwen's Beijing, Singapore, Hong Kong and Virginia regions plus its Token Plan subscription. Those are separate accounts, not mirrors: a key from one is normally refused by another, and the form says so. Where a vendor hands out per-workspace addresses we cannot list, you can type one in. Local runtimes need no API key.
- Model thinking off by default — hybrid reasoning models (e.g. DeepSeek V4) reply fast without billing hidden reasoning tokens; a per-provider control turns thinking back on at five effort levels (Low / Medium / High / XHigh / Max), mapped to each vendor's native parameter (Claude, Gemini, DeepSeek, Zhipu, Qwen, SiliconFlow, OpenRouter, opencode). For a custom endpoint or a local runtime, what the server accepts is decided by software you chose, so you pick the parameter yourself — or leave it sending nothing, which is the default
...
- Update checking (top right of the settings page) — a button that asks GitHub whether a newer release exists, and offers the `.zip` for download if there is one. Only when you press it; nothing runs on its own, and nothing about you is sent.
...
Chrome only auto-updates extensions it installed itself; a self-hosted one can be installed that way only under enterprise policy on Windows and macOS, and an unpacked extension is never auto-updated at all. So updating is manual: download the new `.zip`, unzip it over the old folder, and press Reload on `chrome://extensions`. The top right of the settings page will tell you when there is one, and hand you the file.
